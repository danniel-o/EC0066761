...
if __name__ == '__main__':
    ...
    sys.stdout = Logger('./RSEC_bound_log.txt')

    ''' 创建tsp文件 '''
    RSEC_bound_iter = [0.001, 0.005, 0.01, 0.05, 0.1, 0.5]
    ex_num = 50       # 重复实验的次数
    ...
    for R_SEC_bound in RSEC_bound_iter:
        _clear()
    ...
    '''RSEC_bound_iter 存储用于绘图的数据'''
    f = open('RSEC_bound_iter_H_energy_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_result)
    f.close()

    f = open('RSEC_bound_iter_H_rsec_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(RSEC_result)
    f.close()

    " 查错用 "
    f = open('RSEC_bound_iter_e_all_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all)
    f.close()

    f = open('RSEC_bound_iter_e_all1_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all1)
    f.close()

    f = open('RSEC_bound_iter_e_all2_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all2)
    f.close()

    f = open('RSEC_bound_iter_e_all3_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
