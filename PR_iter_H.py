...
def tsp_generate():
    ...
    _tsp_path = './assets/PR_iter_H'      #!  测试用
...
if __name__ == '__main__':
    ...
    sys.stdout = Logger('./PR_log.txt')

    ''' 创建tsp文件 '''
    PR_iter = [1, 1.5, 2, 2.5, 3, 3.5]
    ex_num = 50       # 重复实验的次数
    ...
    '''PR_iter 存储用于绘图的数据'''
    f = open('PR_iter_H_energy_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_result)
    f.close()

    f = open('PR_iter_H_rsec_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(RSEC_result)
    f.close()

    " 查错用 "
    f = open('PR_iter_e_all_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all)
    f.close()

    f = open('PR_iter_e_all1_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all1)
    f.close()

    f = open('PR_iter_e_all2_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all2)
    f.close()

    f = open('PR_iter_e_all3_50.csv', 'w', encoding='utf-8', newline='')
    writer = csv.writer(f)
    writer.writerows(E_all3)
    f.close()

    # f = open('N_iter_e_all_50.csv', 'w', encoding='utf-8', newline='')
    # writer = csv.writer(f)
    # writer.writerows(E_all)
    # f.close()

    # f = open('N_iter_rsec_all_50.csv', 'w', encoding='utf-8', newline='')
    # writer = csv.writer(f)
    # writer.writerows(RSEC_all)
    # f.close()
